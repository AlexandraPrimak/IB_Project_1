...
# read GTEX proteomics data
annotation_gtex <- read_excel("/Users/alexandraprimak/Desktop/Table_S1_gene_info_at_protein_level.xlsx" ,
                              sheet = 1 ) %>% as.data.table()


# TS scores
ts_scores <- read_excel("/Users/alexandraprimak/Desktop/Table_S1_gene_info_at_protein_level.xlsx" ,  
                        sheet = 11 ) %>% as.data.table
...
enrichment_results_prot <- enrichment_results[geneID %in% unique(tss$geneID) ,  c("geneID", "Protein_category") ]
enrichment_results_prot <- enrichment_results_prot[!duplicated(enrichment_results_prot)]


tss <- merge(tss, enrichment_results_prot, by = "geneID", all.x = T)
tss_df <- tss[ !is.na(TSscore) , c("geneID", "tissue", "TSscore", "Protein_category" )] # subset only trusted values

unique(tss$Protein_category)
#tss_df[, category := "other"]
tss_df[Protein_category == "prt_hk", category := "house-keeping"] # this is how we can assign category
tss_df[TSscore >= 2.5, category := "tissue enriched"] # this is how we can assign category
tss_df[TSscore >= 4 & Protein_category == "prt_specific" , category := "tissue specific"] # this is how we can assign category


tss_df[ , dis_gene := ( geneID %in% unique(hpo_gene$geneID) ) ]
tss_df[is.na(category), category := "other"]

ggplot(tss_df, aes( x = TSscore, fill = dis_gene))+
...
os[ , prop := N/ total] # видимо, тут пропорцию надо изменить
...
p4 <- ggplot(os, aes( dis_gene , prop, fill = category)) + 
  geom_bar(stat = 'identity') +
  coord_flip()+
  scale_y_continuous(labels=scales::percent) + 
  scale_fill_pander() + 
...
p6 <- ggplot(vo[dis_gene != F], aes(category, prop, fill = dis_gene)) + geom_bar(stat = 'identity', position = 'dodge') +
  geom_errorbar(aes(ymin = ci_l, ymax = ci_h), position = position_dodge(.9), width = .3) +
  scale_y_continuous(labels=scales::percent) + 
  scale_fill_pander() + 
  labs(y = 'Proportion of genes', x = 'Tissue specifisity class') +
...
p5 <- ggplot(enr_fisher[dis_gene != F], aes(reorder(category, OR), log_odds)) + 
  geom_point() +
...
p4 + p5 + p6 + plot_layout(widths = c(c(1, 1, 1)))
